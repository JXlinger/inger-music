<template>
    <view class="index">
        <search></search>
        <swiper class="swiper" indicator-dots="true" indicator-color="rgba(0,0,0,.3)" indicator-active-color="rgba(26, 173, 25)" autoplay="true" interval="3000" circular="true">
            <swiper-item v-for="(banner, index) in bannerList" :key="index"><image :src="banner.picUrl" mode="widthFix"></image></swiper-item>
        </swiper>
        ...
        <view class="hot_v">
            <template v-for="(item, index) in recommHotList">
                <view @tap="toSonglist(item.id)" :key="index" class="hot_v_item">
                    <view class="hot_cover"><image :src="item.cover" mode="widthFix"></image></view>
